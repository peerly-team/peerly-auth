...
public abstract class LoginIntegrationTestBase : IAsyncLifetime
{
    ...
    protected async Task<long> AddActiveSessionInDbAsync(long userId)
    {
        await using var connection = await Fixture.DataSource.OpenConnectionAsync();
        ...
        command.Parameters.AddWithValue("refreshTokenHash", Guid.NewGuid().ToString("N"));
    ...
    private async Task<string> HashPasswordAsync(string password)
    {
        ...
        var hashServiceType = typeof(LoginCommand).Assembly.GetType(
        ...
        var hashTask = (Task<string>?)hashAsyncMethod.Invoke(hashService, [password, CancellationToken.None])
